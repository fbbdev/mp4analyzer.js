...
build/mp4analyzer.min.js : build/mp4analyzer.unwrapped.js
	${CLOSURE_COMMAND} --language_in ECMASCRIPT5_STRICT --compilation_level SIMPLE_OPTIMIZATIONS --js build/mp4analyzer.unwrapped.js --js_output_file build/mp4analyzer.min.js --create_source_map build/mp4analyzer.min.js.map --output_wrapper '(function(){%output%}());'

build/mp4analyzer.opt.js : build/mp4analyzer.unwrapped.js
	${CLOSURE_COMMAND} --language_in ECMASCRIPT5_STRICT --compilation_level ADVANCED_OPTIMIZATIONS --warning_level VERBOSE --externs externs.js --js build/mp4analyzer.unwrapped.js --js_output_file build/mp4analyzer.opt.js --create_source_map build/mp4analyzer.opt.js.map --output_wrapper '(function(){%output%}());'
...
clean:
	rm -rf build/mp4analyzer.unwrapped.js build/mp4analyzer.js build/mp4analyzer.min.js build/mp4analyzer.opt.js build/*.js.map
